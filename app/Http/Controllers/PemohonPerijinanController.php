...
class PemohonPerijinanController extends Controller
{
	public function izin($slug)
	{
		...
		if($perijinan->slug_perijinan == 'izin-pendidikan'){
			...
			return view('pemohon.pendidikan.index', compact('perijinan','getkecamatan','status','dataIjinPendidikan','jenis_izin'));
			...
			return view('pemohon.lingkungan.index', compact('perijinan','getkecamatan','datastatus','dataIjinLingkungan','jenis_izin'));
		}elseif($perijinan->slug_perijinan == 'siujk'){
			$dataIjinSiujk = IjinSiujk::with('pemohon','datastatus')->get();
			return view('pemohon.pupr.index', compact('perijinan','getkecamatan','status','dataIjinSiujk','jenis_izin'));
		}else{}
	}
	...
	public function detail($slug,$id)
	{
		...
		if($perijinan->slug_perijinan == 'izin-pendidikan')
		{
			...
			return view('pemohon.pendidikan.detail', compact('ijin_pendidikan','perijinan'));
		}elseif ($perijinan->slug_perijinan == 'izin-lingkungan') {
			$ijin_lingkungan = IjinLingkungan::with('district','village','datastatus','pemohon')->find($id);
			return view('pemohon.lingkungan.detail', compact('ijin_lingkungan','perijinan'));
		}elseif($perijinan->slug_perijinan == 'siujk'){
			$ijin_siujk = IjinSiujk::with('village','datastatus','pemohon','jenissbu','jenisskt','jenisska')->find($id);
			// dd($ijin_siujk);
			return view('pemohon.pupr.detail', compact('ijin_siujk','perijinan'));
		}
	}
