...
<div class="row">
	<div class="col-md-12">
		...
		<div class="tile">
			<div class="tile-body">
				<button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#tambahIjinlingkungan" aria-expanded="false" aria-controls="tambahIjinlingkungan">
					Tambah
				</button>
			</div>
		</div>
		<div class="collapse" id="tambahIjinlingkungan">
			<div class="card card-body">
				{{ Form::open(['route'=>['frontOffice.perijinan.post',$perijinan->slug_perijinan],'method'=>'POST','files'=>true]) }}
				@include('front_office.lingkungan.tambah_data_ijin')
				{{ Form::close() }}
			</div>
		</div>
		<div class="tile">
			<div class="tile-body">
				<table class="table table-bordered ijin_lingkungan">
					...
					<tbody>
						@foreach($dataIjinLingkungan as $dil)
						<tr>
							<td>{{ $loop->iteration }}</td>
							<td>{{ $dil->tanggal_daftar }}</td>
							@if($dil->pemohon_id == 0)
							<td>-</td>
							<td>-</td>
							@else
							<td>{{ $dil->pemohon->nik }}</td>
							<td>{{ $dil->pemohon->nama_lengkap }}</td>
							@endif
							<td>{{ $dil->datastatus->nama_status }}</td>
							<td>
								<a href="{{ route('pemohon.perijinan.detail',[$perijinan->slug_perijinan,$dil->id]) }}" class="btn btn-block btn-primary"><i class="fa fa-info-circle" aria-hidden="true"></i></a>
							</td>
							@include('front_office.lingkungan.ubah_status')
							<td>
								{{ Form::model($dil, ['route'=>['frontOffice.perijinan.deleteperijinan', $perijinan->slug_perijinan,$dil->id],'method'=>'POST']) }}
									@method('DELETE')
